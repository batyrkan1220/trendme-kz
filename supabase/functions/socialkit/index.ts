                ...
                messages: [
                  {
                    ...
                    content: analysisLang === "kk"
                      ? `Сен — TikTok контентін талдау бойынша сарапшысыз. Бейнені барлық қолжетімді ақпарат негізінде (сипаттама, статистика, пікірлер) талдап, құрылымдалған талдау бер.

ЕСКЕРТУ — ЕҢ МАҢЫЗДЫ ЕРЕЖЕ: Видеоның мәнін, мағынасын ЕШҚАШАН өзгертпе. Видеода не айтылса, не көрсетілсе — дәл соны жаз. Ойдан шығарма, қоспа, өзгертпе. Тек видеодағы нақты ақпаратты 1:1 дәл бер. Егер бірдеңе белгісіз болса — ойлап шығарудың орнына "белгісіз" деп жаз.

МАҢЫЗДЫ: ТІЛДІ ҚАЗАҚША ЖАУАП БЕР. Тек video_analysis функциясын шақыр, қосымша мәтінсіз.`
                      : `Ты — эксперт по анализу контента в TikTok. Проанализируй видео на основе всей доступной информации (описание, статистика, комментарии) и верни структурированный анализ.

ВНИМАНИЕ — САМОЕ ВАЖНОЕ ПРАВИЛО: НЕ ИЗМЕНЯЙ смысл и содержание видео. Описывай РОВНО то, что есть в видео — ни больше, ни меньше. Не додумывай, не приукрашивай, не подменяй смысл. Передай суть видео 1:1 точно как в оригинале. Если что-то неизвестно — напиши "неизвестно" вместо выдумки.
...
ВАЖНО: Отвечай ТОЛЬКО вызовом функции video_analysis, без лишнего текста.`
                  },
                  {
                    role: "user",
                    content: analysisLang === "kk"
                      ? `Осы TikTok бейнесін талда. Видеоның мәнін 100% дәл сақта, ешнәрсе өзгертпе.\n\nURL: ${video_url || "N/A"}\n\n${contextParts.join("\n\n")}`
                      : `Проанализируй это TikTok видео. Передай смысл видео 100% точно, ничего не изменяй.\n\nURL: ${video_url || "N/A"}\n\n${contextParts.join("\n\n")}`
                  }
                ],
                tools: [
                  {
