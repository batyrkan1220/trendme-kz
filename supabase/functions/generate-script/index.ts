...
ФОРМАТТАУ ЕРЕЖЕЛЕРІ (міндетті түрде сақта!):
- Әр бөлімді бос жолмен бөл
- Эмодзилер қолдан — бірақ артық емес, тек бөлім басында
- Хуктердің әрқайсысын нөмірлеп, жеке абзацта жаз
- Сценарийді бөлімдерге бөл: 🎬 ХУК → 📖 НЕГІЗГІ БӨЛІМ → 🎯 CTA
- Түсірілім нұсқауларын [квадрат жақшада] жаз, бөлек жолда
- Қысқа абзацтар — бір абзацта 1-2 сөйлем
- Маңызды сөздерді **қалың** жаз
- Тізімдерді қолдан, ұзын абзацтардан аулақ бол

МАЗМҰН ЕРЕЖЕЛЕРІ:
1. Түпнұсқаның құрылымы мен табыс формуласын сақта
2. Контентті бейімде — сөзбе-сөз көшірме емес
3. Хуктің 3 нұсқасын ұсын (назар аударатын бірінші сөйлем)
4. Толық сценарийді жаз — түсірілімге дайын
5. Сөйлесу стилін, қысқа сөйлемдерді қолдан
6. Соңында CTA (әрекетке шақыру) қос
...
ПРАВИЛА ФОРМАТИРОВАНИЯ (обязательно соблюдай!):
- Разделяй каждый блок пустой строкой для воздушности
- Используй эмодзи — но умеренно, только в начале секций
- Каждый вариант хука нумеруй и пиши отдельным абзацем
- Дели сценарий на чёткие секции: 🎬 ХУК → 📖 ОСНОВНАЯ ЧАСТЬ → 🎯 CTA
- Указания для съёмки пиши [в квадратных скобках], на отдельной строке
- Короткие абзацы — 1-2 предложения максимум
- Важные слова выделяй **жирным**
- Используй списки, избегай длинных сплошных абзацев
...
4. Напиши полный адаптированный сценарий, готовый к съёмке
5. Используй разговорный стиль, короткие предложения
6. Добавь CTA (призыв к действию) в конце
