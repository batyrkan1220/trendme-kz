...
export const VideoCard = forwardRef<HTMLDivElement, VideoCardProps>(function VideoCard({
  ...
  return (
    <div className={`group rounded-2xl overflow-hidden hover:shadow-lg transition-shadow duration-200 relative flex flex-col ${darkMode ? "bg-[#1a1a1a] border border-white/10" : "bg-card border border-border/40"}`} onMouseEnter={handlePreload} onMouseLeave={handlePreloadCancel} onTouchStart={handlePreload}>
      {/* Video area */}
      <div className="relative aspect-[9/16] bg-black overflow-hidden rounded-2xl m-1.5">
        {playingId === video.id ? (
          <>
            {loadingPlay ? (
      ...
      <div className="grid grid-cols-4 px-1.5 py-1.5">
        ...
        <span className="flex items-center justify-center gap-1">
          <Heart className={`h-3 w-3 shrink-0 ${darkMode ? "text-white/40" : "text-muted-foreground"}`} />
          <span className={`text-[10px] font-bold ${darkMode ? "text-white" : "text-foreground"}`}>{fmt(Number(video.likes))}</span>
        </span>
        <span className="flex items-center justify-center gap-1">
          <MessageCircle className={`h-3 w-3 shrink-0 ${darkMode ? "text-white/40" : "text-muted-foreground"}`} />
          <span className={`text-[10px] font-bold ${darkMode ? "text-white" : "text-foreground"}`}>{fmt(Number(video.comments))}</span>
        </span>
        <span className="flex items-center justify-center gap-1">
          <Share2 className={`h-3 w-3 shrink-0 ${darkMode ? "text-white/40" : "text-muted-foreground"}`} />
          <span className={`text-[10px] font-bold ${darkMode ? "text-white" : "text-foreground"}`}>{fmt(Number(video.shares || 0))}</span>
        </span>
      </div>
